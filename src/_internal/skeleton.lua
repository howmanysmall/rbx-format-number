...
local function notation_to_skeleton(notation)
	...
	if notation.type == enums._Internal.NotationType.SCIENTIFIC then
		...
		if notation.minExponentDigits ~= 1 then
			result ..= "/*" .. string.rep("e", notation.minExponentDigits)
		end
...
local function precision_to_skeleton(precision)
	...
	if frac_s then
		...
		if sig_s then
			result ..= "/" .. sig_s
			if
				precision.type == enums._Internal.PrecisionType.FRACTION_SIGNIFICANT
				and precision.sourcedWithSignificantDigits
			then
...
local function int_width_to_skeleton(int_width)
	...
	elseif int_width.max == 0 and int_width.min == 0 then
		...
		result = "integer-width/" .. string.rep("#", int_width.max - int_width.min) .. string.rep("0", int_width.min)
	end
...
local function skeleton_to_precision(str)
	...
	if max_frac_str then
		...
		if string.match(c_str, "^[%*%+]") and min_frac_str == max_frac_str then
			max_frac = -1
		...
		if string.sub(c_str, 1, 1) == "/" then
			...
			local rp_incl = false
			max_sig_str, min_sig_str, rp = string.match(c_str, "^/((@+)#*)([%*%+rs]?)$")

			if
				not max_sig_str
				or rp == "" and min_sig_str ~= "@"
				or (rp == "*" or rp == "+") and min_sig_str ~= max_sig_str
			then
				return nil
			end
			...
			return table.freeze({
				...
				roundingPriority = if rp == "r" then enums.RoundingPriority.RELAXED else enums.RoundingPriority.STRICT;
...
local function skeleton_to_int_width(str)
	...
	max_int_str, min_int_str = string.match(str, "^integer%-width/(#*(0*))$")

	-- sanity check
	if not max_int_str or max_int_str == "" or #max_int_str > 999 or #min_int_str > 999 then
		return nil
	end
...
local function skeleton_to_int_width_concise(str)
	return if string.match(str, "^0+$") and #str < 1000
		then table.freeze({
			min = #str;
			max = -1;
		})
...
function skeleton.to_option_linked_list(skeleton_string)
	...
	for pos, token in string.gmatch(skeleton_string, "()(%S+)") do
		...
		if not r_value then
			r_value = skeleton_to_scientific_notation(token) or skeleton_to_scientific_notation_concise(token)
			if r_value then
				r_key = "notation"
			end
		end
		...
		if not r_value then
			r_value = skeleton_to_int_width(token) or skeleton_to_int_width_concise(token)
			if r_value then
				r_key = "integerWidth"
			end
		end
		...
		if not r_key or keys[r_key] then
			return false,
				string.format(
					"number skeleton syntax error near '%*' at position %d",
					(string.gsub(token, "'", "\\'")),
					pos
				)
		end
