...
function format.round_sig(
	...
	if fmt_n > sig then
		...
		elseif rounding_mode == enums.RoundingMode.HALF_EVEN then
			resolved_rounding_midpoint = if sig <= 0 then 0 else fmt[sig] % -2
...
function format.resolve_int_frac(
	...
	if min_grouping and marker - i + 1 >= min_grouping then
		...
		intg = string.reverse((string.gsub(string.reverse(intg), "...", grouping_repl, (marker - i) / 3)))
	end
...
function format.format_expt(expt, symbols, min_int_digits, disp_sign_at)
	...
	return symbols[enums.ENumberFormatSymbols.kExponentialSymbol] .. disp_sign .. exp_str
end
...
function format.resolve_with_notation(fmt, fmt_n, scale, notation, symbols)
	...
	if fmt_n <= 0 then
		if notation.type == enums._Internal.NotationType.SCIENTIFIC then
			fmtd_expt = format.format_expt(0, symbols, notation.minExponentDigits, notation.displayExponentSignAt)
		end
	else
...
function format.format_unsigned_finite(fmt, fmt_n, scale, is_negt, internal_settings): string
	...
	re_min_sig, re_max_sig = formatter_settings.resolve_min_max_sig(internal_settings.precision, decimal_marker)

	fmt_n, incr_marker = format.round_sig(fmt, fmt_n, re_max_sig, is_negt, internal_settings.roundingMode)
	...
	elseif rounded_to_zero then
		-- for convenince
		re_min_sig, re_max_sig = formatter_settings.resolve_min_max_sig(internal_settings.precision, 1)
		decimal_marker = 1
	end
	...
	if internal_settings.alwaysDisplayDecimal or fmtd_frac ~= "" then
		result ..= symbols[enums.ENumberFormatSymbols.kDecimalSeparatorSymbol] .. fmtd_frac
	end
