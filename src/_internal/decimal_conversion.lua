...
function decimal_conversion.from_double(double: number): ({number}, number, number)
	...
	local sigt_int, sigt_frac, expt = string.match(tostring(double), "^(%d+)%.?(%d*)e?([+-]?%d*)$")
	local sigt, sigt_incr = string.match(sigt_int .. sigt_frac, "^0*(%d-)(0*)$")

	result_fmt = {string.byte(string.gsub(sigt, ".", NUMBER_BYTE_CONVERSION), 1, -1)}
	result_n = #result_fmt
