...
function class.create_init_function(
	...
	return function(data, as_string)
		...
		result_object_meta.as_string = as_string or string.sub(tostring(result_object), 11)
	...
	elseif string.sub(type_name, 1, 1) == "{" then
		...
		if type(value) == "table" then
			...
			for i, v in coerced_value do
				if type(v) ~= t then
					error(
						string.format("Values inside the table argument must be a %s, index %d got %s", t, i, type(v)),
						3
					)
				end
			end
