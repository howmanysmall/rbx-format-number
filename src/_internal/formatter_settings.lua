...
function formatter_settings.resolve_settings(settings_)
	...
	if settings_.precision then
		...
		resolved_settings.precision = if is_compact_notation
			then table.freeze({
	...
	if settings_.sign then
		resolved_settings.displaySignAt = formatter_settings.generate_from_sign_enum(settings_.sign)
	else
		resolved_settings.displaySignAt = formatter_settings.generate_from_sign_enum(enums.SignDisplay.AUTO)
	end
	resolved_settings.alwaysDisplayDecimal = settings_.decimal == enums.DecimalSeparatorDisplay.ALWAYS
