...
local function resolve_nf_data(data)
	return _internal.formatter_settings.resolve_settings(_internal.formatter_settings.linked_list_to_dict(data))
end
...
function NumberFormatter_methods.Format(self: NumberFormatter, value: number): string
	...
	elseif value == math.huge or value == -math.huge then
		...
		if value == 0 then
			...
			is_negt = value < 0
			fmt, fmt_n, decimal_marker = _internal.decimal_conversion.from_double(math.abs(value))
		end

		result, is_zero =
			_internal.format.format_unsigned_finite(fmt, fmt_n, decimal_marker, is_negt, internal_settings)
	end
	...
	result = _internal.format.display_sign(
		...
		is_negt,
		is_zero,
		internal_settings.displaySignAt,
		internal_settings.symbols
	)
...
function NumberFormatter_methods.ToSkeleton(self: NumberFormatter): (boolean, string)
	...
	return _internal.skeleton.settings_to_skeleton(_internal.formatter_settings.linked_list_to_dict(c_self))
end
